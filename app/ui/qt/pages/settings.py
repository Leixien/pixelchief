...
class SettingsPage(QWidget):
    ...
    def _build_earthquake_card(self):
        ...
        card.card_layout.addWidget(SectionTitle('Auto-restart'))
        repeat_hint = QLabel('When a session ends, wait a random pause inside this range and start another one, until you press Stop. Set the minimum to 0 to run a single session as before. Needs a timed run: "Run until maxed" never ends on its own, so there is nothing to restart.')
        repeat_hint.setWordWrap(True)
        repeat_hint.setStyleSheet(f'''color: {TOKENS['text_muted']};''')
        card.card_layout.addWidget(repeat_hint)
        repeat_row = QHBoxLayout()
        self._repeat_min = QSpinBox()
        self._repeat_min.setRange(0, RANDOM_MINUTES_CAP)
        self._repeat_min.setFixedWidth(88)
        repeat_row.addWidget(self._repeat_min)
        repeat_to = QLabel('to')
        repeat_to.setStyleSheet(f'''color: {TOKENS['text_muted']};''')
        repeat_row.addWidget(repeat_to)
        ...
        repeat_row.addWidget(repeat_unit)
        repeat_row.addStretch()
        card.card_layout.addLayout(repeat_row)
        btn_row = QHBoxLayout()
    ...
    def _reload_earthquake(self):
        ...
        self._repeat_max.setValue(settings.repeat_pause_max)
    ...
    def _on_save(self):
        ...
        self._repeat_min.setValue(saved.repeat_pause_min)
        self._repeat_max.setValue(saved.repeat_pause_max)
