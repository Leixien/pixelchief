...
class _AdbBoundaryUnavailable(RuntimeError):
    '''No validated deployment boundary is available for this battle.'''
...
class AttackStrategy:
    ...
    def _deployment_snapshot(self, stage: str, frame: Any = None) -> None:
        if not self.input.window_service.use_adb:
            return
        try:
            directory = get_user_app_data_dir() / 'debug'
            ensure_dir(directory)
            if stage == 'before':
                self._deployment_overlay = frame.copy()
                self._deployment_previous = None
                """ logger.info('ADB deployment: profile=%s frame=%sx%s anchors=%s files=%s',
                            self.config.aspect_key, frame.shape[1], frame.shape[0],
                            self._scaled_deployment_data(), directory) """
            elif frame is None:
                frame = self.input.window_service.screenshot()
            images = [(f'deployment_{stage}.png', frame)]
            if stage == 'before':
                # Reset the previous raid's image; the log identifies completed after captures.
                images.append(('deployment_after.png', frame))
            overlay = getattr(self, '_deployment_overlay', None)
            if overlay is not None:
                images.append(('deployment_path.png', overlay))
    ...
    def _deployment_point(self, label: str, x: int, y: int, *, drag: bool = False) -> None:
        if not self.input.window_service.use_adb:
            return
        try:
            point = self.input._clamp_to_capture(x, y)
            #logger.info('ADB deployment %s: requested=(%s,%s) clamped=%s', label, x, y, point)
            overlay = getattr(self, '_deployment_overlay', None)
            if overlay is None:
                return
            previous = getattr(self, '_deployment_previous', None)
            if drag and previous is not None:
                # Segment endpoints only; the input service generates the actual Bezier curve.
                cv2.line(overlay, previous, point, (0, 255, 255), 2)
            cv2.circle(overlay, point, 6, (0, 0, 255), 2)
            cv2.putText(overlay, label, (max(0, min(point[0], overlay.shape[1] - 180)),
                                       max(16, min(point[1] - 8, overlay.shape[0] - 8))),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
            self._deployment_previous = point if drag else None
        except Exception:
            logger.warning('ADB deployment point diagnostic failed', exc_info=True)

    def _adb_deployment_failure(self, reason: str) -> None:
        logger.warning('ADB deployment stopped: %s', reason)
        self._deployment_snapshot('after')
        raise _AdbBoundaryUnavailable(f'ADB deployment stopped: {reason}')

    def _adb_border_points(self) -> tuple[Any, List[Tuple[int, int]]]:
        points = []
        for attempt in range(3):
            ...
            if cached is not None and self.vision._deployment_camera_stable(cached[0], frame):
                points = cached[1]
                #logger.info('ADB deployment: reusing %d validated points with stable camera', len(points))
                break
            points = self.vision._deployment_border(frame, self._deployment_overlay)
            if points:
                self._adb_border_cache = (frame.copy(), points)
                break
            #logger.info('ADB deployment: boundary acquisition attempt %d/3 failed', attempt + 1)
            if attempt == 2:
                break
            if (not getattr(self, '_adb_border_probe_used', False)
                    and not getattr(self, '_adb_deployment_started', False)):
                self._adb_border_probe_used = True
                h, w = frame.shape[:2]
                self._deployment_point('border probe', w // 2, h // 2)
                self.input.click(w // 2, h // 2, pause=.2, rand=False)
            if self.stop_event:
                ...
                time.sleep(.3)
        if not points:
            self._adb_deployment_failure('red boundary unavailable or ambiguous')
        self._deployment_previous = None
        for point in points:
            self._deployment_point('border drop', *point)
    ...
    def _adb_deploy_heroes(self) -> None:
        ...
        for name in names:
            ...
            self._adb_deployment_started = True
            self.input.click(*point, pause=.2, rand=False)
            if siege:
                siege_selected = True
            else:
                self.input.click(x, y, pause=.2, rand=False)
    ...
    def deploy_heroes(self, frame):
        ...
        if ix:
            ...
            self._deployment_point('siege select', ix, iy)
            self._deployment_point('siege drop (+/-15px)', *deploy_point)
        ...
        for hero in heroes:
            ...
            if not bx:
                continue
            deploy_point = self._get_hero_deploy_point(frame)
            self._deployment_point(f'{hero} select', bx, by)
...
'''
    ...
    def _deploy_diamond_perimeter_troop(self, frame, template_name, stop_event = None, *, count = _EDRAG_COUNT, delay = _EDRAG_DELAY, point = None):
        ...
        if ev and ev.wait(0.2):
            return True
        if self.input.window_service.use_adb:
            self._adb_deploy_selected(template_name, count)
            return True
...
class TroopSpamStrategy(AttackStrategy):
    ...
    def execute(self, frame, stop_event = None):
        ...
        if tx is None:
            ...
            return False
        self._deployment_point(f'{self.troop_name} select', tx, ty)
        self.input.click(tx, ty, pause = 0.3, rand = False)
        if ev and ev.wait(0.2):
            return True
        if self.input.window_service.use_adb:
        ...
        self._deployment_point('DOWN', curr_x, curr_y, drag=True)
        try:
            self.input.mouse_down(curr_x, curr_y)
        except Exception:
            if self.input.window_service.use_adb:
                logger.exception('ADB deployment DOWN failed')
                self._deployment_snapshot('after')
            raise
        ...
        try:
            ...
            for i in range(len(ordered_corners) - 1):
                ...
                self._deployment_point(f'MOVE {i + 1}', target_x, target_y, drag=True)
                if self.input.window_service.use_adb:
                    logger.info('ADB deployment segment %s: duration=%.3fs', i + 1, duration)
                self.input.human_move(curr_x, curr_y, target_x, target_y, duration = duration)
                curr_y = target_y
                curr_x = target_x
            self._deployment_point('UP', curr_x, curr_y, drag=True)
            self.input.mouse_up(curr_x, curr_y)
            if ev and ev.is_set():
                return True
            frame = self.input.window_service.screenshot()
