...
'''
    ...
    def _start_wall_debug(self):
        try:
            stamp = time.strftime('%Y%m%d_%H%M%S')
            suffix = time.time_ns() % 1000000
            self._wall_debug_dir = (
                get_user_app_data_dir() / 'debug' / 'walls' / f'{stamp}_{suffix:06d}')
            self._wall_debug_dir.mkdir(parents = True, exist_ok = True)
            self._wall_debug_seq = 0
            logger.info('Wall upgrade: diagnostics saved to %s', self._wall_debug_dir)
        except Exception:
            self._wall_debug_dir = None
            logger.debug('Wall upgrade: could not create diagnostic directory', exc_info = True)


    def _wall_debug_save(self, label, frame = None, point = None):
        if self._wall_debug_dir is None:
            return
        try:
            if frame is None:
                frame = self.window.screenshot()
            ...
            if point is not None:
                ...
                cv2.line(image, (x - radius, y), (x + radius, y), (0, 0, 255), 2)
                cv2.line(image, (x, y - radius), (x, y + radius), (0, 0, 255), 2)
                cv2.putText(image, f'click ({x}, {y})', (max(0, x - 90), max(25, y - radius - 8)),
                            cv2.FONT_HERSHEY_SIMPLEX, .6, (0, 0, 255), 2)
            self._wall_debug_seq += 1
            safe = ''.join(c if c.isalnum() or c in '-_' else '_' for c in label)
            path = self._wall_debug_dir / f'{self._wall_debug_seq:03d}_{safe}.jpg'
    ...
    def _wall_debug_click(self, label, x, y, *, pause = 0.3):
        before = self.window.screenshot()
        self._wall_debug_save(f'{label}_before', before)
        self._wall_debug_save(f'{label}_click', before, (x, y))
        self.input.click(x, y, pause = pause, rand = False)
        after = self.window.screenshot()
        self._wall_debug_save(f'{label}_after', after)
        return after


    def _wall_menu_scroll(self):
        if self.window.use_adb:
            width, height = self.window.get_outer_pixel_size()
            x = min(width - 1, max(0, self._wall_menu_scroll_point()[0]))
            self.window.adb.swipe(
                x, int(height * .58), x, int(height * .32), duration_ms = 350)
        else:
    ...
    def _should_upgrade_walls(self):
        ...
        raw = [group.text for group in groups]
        source = 'single'
        if triplet is None:
            triplet = self._read_hud_triplet_stable()
            source = 'stable' if triplet is not None else 'none'
        full_icon = gx is not None or ex is not None
        if triplet is None:
            try:
                debug_dir = get_user_app_data_dir() / 'debug'
                debug_dir.mkdir(parents = True, exist_ok = True)
                debug_path = debug_dir / 'wall_threshold_ocr_failed.jpg'
                cv2.imwrite(str(debug_path), frame,
                            [cv2.IMWRITE_JPEG_QUALITY, 90])
            except Exception:
                debug_path = None
                logger.debug('Wall upgrades: failed to save threshold OCR frame',
                             exc_info = True)
            logger.warning('Wall upgrades: threshold check raw=%s source=%s '
                           'parsed=None threshold=%s full_icon=%s decision=%s '
                           'screenshot=%s', raw, source, threshold, full_icon,
                           full_icon, debug_path)
            return full_icon
        self._last_hud_triplet = triplet
        (gold, elixir, _) = triplet
        reached = threshold > 0 and (gold >= threshold or elixir >= threshold)
        decision = full_icon or reached
        logger.info('Wall upgrades: threshold check raw=%s source=%s parsed '
                    'gold=%s elixir=%s dark=%s threshold=%s full_icon=%s '
                    'decision=%s', raw, source, gold, elixir, triplet[2],
                    threshold, full_icon, decision)
        return decision
...
deselect, which would eat the upcoming Attack click.'''
        if self.window.use_adb:
            ...
            wall_ui_open = bool(
                VisionService.find_active_addwall(frame, region = bottom_roi)[0]
                or VisionService.find_active_removewall(frame, region = bottom_roi)[0]
                or self.vision.find_template(frame, 'upgrademore.png', region = bottom_roi)[0]
                or self.vision.find_template(frame, 'okay.png')[0])
            if get_template_path('needgold_x.png').exists():
                wall_ui_open = wall_ui_open or bool(
                    self.vision.find_template(frame, 'needgold_x.png')[0])
            if wall_ui_open:
                self._wall_debug_click(
                    'deselect_empty', *self.config.get_point('empty'))
            return
        self._dismiss_open_popup()
        self._wall_debug_click('deselect_empty', *self.config.get_point('empty'))
    ...
    def _maybe_upgrade_walls(self, upgrade_walls):
        ...
        for _ in range(4):
            self._check_stop()
            if completed and not self._should_upgrade_walls():
                break
            if self._upgrade_walls():
                completed += 1
    ...
    def _emit_state(self, state = None, builders = None, lab = None, storages = None,
                    note = None, walls_upgraded = None):
        '''Push a live-state update to the UI panel (best-effort, never raises).'''
        cb = getattr(self, '_state_callback', None)
        if not cb:
            return None
        try:
            cb({ 'state': state, 'builders': builders, 'lab': lab,
                 'storages': storages, 'note': note,
    ...
    def _upgrade_walls_pick_resource_and_okay(self):
        ...
        for read_attempt in range(2):
            self._wall_debug_save(
                f'payment_read_{read_attempt + 1}_g{pair.gold.redness:.2f}_'
                f'e{pair.elixir.redness:.2f}', frame)
        ...
        if pair.elixir.redness < _WALL_COST_REDNESS_THRESHOLD and pair.elixir.cost_roi_xywh and pair.elixir.center:
            logger.info('Wall upgrade: paying with elixir (redness %.2f)', pair.elixir.redness)
            self._wall_debug_click('pay_elixir', *pair.elixir.center)
            picked = True
        elif pair.gold.redness < _WALL_COST_REDNESS_THRESHOLD and pair.gold.cost_roi_xywh and pair.gold.center:
            logger.info('Wall upgrade: paying with gold (redness %.2f)', pair.gold.redness)
            self._wall_debug_click('pay_gold', *pair.gold.center)
            picked = True
        frame = self.window.screenshot()
        if frame is None:
            return None
        self._update_config_size(frame)
        self._wall_debug_save(f'payment_after_selection_{"picked" if picked else "none"}', frame)
        if not picked:
            if self._confirm_wall_upgrade(frame):
                return True
            ...
            if not pair.gold.cost_roi_xywh and not pair.elixir.cost_roi_xywh:
                self._wall_debug_save('payment_cost_buttons_missing', frame)
            self._deselect_wall_ui()
            return False
    ...
    def _upgrade_walls(self):
        ...
        for attempt in range(3):
            self.input.click(bx, by, pause = 0.5, rand = False)
            if self.stop_event.wait(0.4):
                return None
            check = self.window.screenshot()
            if check is None:
                continue
            self._update_config_size(check)
            menu_roi = VisionService.top_half_region(check)
            markers = ('addwall.png', 'multiupgrade.png', 'removewall.png', 'upgrademore.png')
            menu_open = any(self.vision.find_template(check, marker, region = menu_roi)[0] for marker in markers)
            if not menu_open:
                menu_open = self._find_wall_row_once() is not None
            logger.info('Wall upgrade: builder click attempt %d/3 menu_open=%s', attempt + 1, menu_open)
            if menu_open:
                break
        if not menu_open:
            logger.warning('Wall upgrade: builder click did not open improvement list; skipping swipes')
            return None
        ...
        candidate = None
        scrolls = 0
        scroll_positions = 24 if self.window.use_adb else _WALL_MENU_SCROLL_STEPS
        for _ in range(scroll_positions * 2):
            self._check_stop()
            ...
            if found:
                ...
                continue
            candidate = None
            if scrolls >= scroll_positions - 1:
                break
            self._wall_menu_scroll()
            scrolls += 1
            if self.window.use_adb:
                logger.info('Wall upgrade: ADB scroll %d/%d completed (%d gestures total)',
                            scrolls, scroll_positions - 1, scrolls)
            if self.stop_event.wait(0.5):
                return None
        if not wall_pt:
            logger.info('Wall upgrade: Wall label OCR not confirmed after %d scroll nudge(s) — skipping this pass', scrolls)
            return None
        self.input.click(*wall_pt, pause = 0.6, rand = False)
        ...
        if not umx:
            ...
            return None
        upgrade_click = (umx, max(0, umy - self.config.scale_scalar(80)))
        logger.info('Wall upgrade: Upgrade More matched at (%d,%d), clicking button center at %s',
                    umx, umy, upgrade_click)
        panel_open = False
        for click_attempt in range(2):
            self.input.click(*upgrade_click, pause = 0.4, rand = False)
            for poll_attempt in range(3):
                ...
                self._update_config_size(frame)
                (fh, fw) = frame.shape[:2]
                mid_roi = (fw // 4, int(fh * .69), fw // 2, int(fh * .23))
                resources = VisionService.find_upgrade_resource_buttons(frame, mid_roi)
                has_multi = bool(self.vision.find_template(
                    frame, 'multiupgrade.png', region=mid_roi,
                )[0])
                has_add = bool(VisionService.find_active_addwall(frame, region=mid_roi)[0])
                panel_open = has_multi or bool(resources) or has_add
                logger.info('Wall upgrade: Upgrade More click %d/2 panel_open=%s '
                            '(multi=%s resources=%s add=%s)', click_attempt + 1,
                            panel_open, has_multi, sorted(resources), has_add)
                if panel_open:
                    break
            ...
            if panel_open:
                break
        if not panel_open:
            logger.warning('Wall upgrade: Upgrade More did not open the wall selection panel')
            return None
        self._start_wall_debug()
        self._wall_debug_save('batch_start', frame)
        # Add walls to the batch while at least one resource can still pay the total.
        added = 0
        for add_attempt in range(_WALL_BATCH_MAX_ADDS):
            self._check_stop()
            frame = self.window.screenshot()
            if frame is None:
                return None
            ...
            pair = VisionService.upgrade_cost_redness_by_resource_icons(frame)
            self._wall_debug_save(
                f'add_try_{add_attempt + 1}_cost_g{pair.gold.redness:.2f}_'
                f'e{pair.elixir.redness:.2f}', frame)
            if not pair.gold.cost_roi_xywh and not pair.elixir.cost_roi_xywh:
                ...
                break
            if (pair.gold.redness >= _WALL_COST_REDNESS_THRESHOLD
                    and pair.elixir.redness >= _WALL_COST_REDNESS_THRESHOLD):
                break
            ...
            if not awx:
                self._wall_debug_save(f'add_wall_try_{add_attempt + 1}_not_found', frame)
                break
            logger.info('Wall upgrade: clicking Add Wall button center at (%d,%d)', awx, awy)
            self._wall_debug_click(f'add_wall_try_{add_attempt + 1}', awx, awy)
            added += 1
            check = self.window.screenshot()
            if check is None:
                return None
            self._update_config_size(check)
            (okay_x, _) = self.vision.find_template(check, 'okay.png')
            controls = VisionService.find_upgrade_resource_buttons(check, mid_roi)
            (next_add_x, _) = VisionService.find_active_addwall(check, region=mid_roi)
            self._wall_debug_save(
                f'add_wall_try_{add_attempt + 1}_validation_'
                f'{"okay" if okay_x else "panel" if controls or next_add_x else "failed"}',
                check)
            if okay_x:
                logger.info('Wall upgrade: confirmation appeared after Add Wall click')
                break
            if not controls and not next_add_x:
                logger.warning('Wall upgrade: panel validation failed after Add Wall click; stopping batch')
                break
        if added == 0:
            self._deselect_wall_ui()
            return None
        ...
        pair = VisionService.upgrade_cost_redness_by_resource_icons(frame)
        self._wall_debug_save(
            f'batch_final_cost_g{pair.gold.redness:.2f}_e{pair.elixir.redness:.2f}', frame)
        (fh, fw) = frame.shape[:2]
        mid_roi = (fw // 4, int(fh * .69), fw // 2, int(fh * .23))
        if (added and pair.gold.redness >= _WALL_COST_REDNESS_THRESHOLD
                and pair.elixir.redness >= _WALL_COST_REDNESS_THRESHOLD):
            ...
            if not rwx:
                self._wall_debug_save('remove_unaffordable_wall_not_found', frame)
                logger.warning('Wall upgrade: both costs are red and Remove Wall was not found — aborting batch')
                self._deselect_wall_ui()
                return None
            logger.info('Wall upgrade: clicking Remove Wall button center at (%d,%d)', rwx, rwy)
            self._wall_debug_click('remove_unaffordable_wall', rwx, rwy, pause = 0.4)
            added -= 1
            frame = self.window.screenshot()
            if frame is None:
                return None
            ...
            pair = VisionService.upgrade_cost_redness_by_resource_icons(frame)
            self._wall_debug_save(
                f'after_remove_cost_g{pair.gold.redness:.2f}_e{pair.elixir.redness:.2f}',
                frame)
            if (not pair.gold.cost_roi_xywh and not pair.elixir.cost_roi_xywh) or (
                    pair.gold.redness >= _WALL_COST_REDNESS_THRESHOLD
                    and pair.elixir.redness >= _WALL_COST_REDNESS_THRESHOLD):
                logger.warning('Wall upgrade: costs still unavailable/red after Remove Wall — aborting batch')
                self._deselect_wall_ui()
                return None
        elif (added > 1
              and pair.elixir.redness >= _WALL_COST_REDNESS_THRESHOLD
              and pair.gold.redness < _WALL_COST_REDNESS_THRESHOLD
              and pair.gold.cost_roi_xywh):
            ...
            if rwx:
                ...
                logger.info('Wall upgrade: clicking Remove Wall button center at (%d,%d)', rwx, rwy)
                self._wall_debug_click('remove_gold_reserve_wall', rwx, rwy, pause = 0.4)
                added -= 1
        logger.info('Wall upgrade: %d wall(s) added to batch', added)
        confirmed = self._upgrade_walls_pick_resource_and_okay()
        if confirmed:
            upgraded = added + 1
            self._walls_upgraded_session += upgraded
            logger.info('Wall upgrade: %d confirmed, %d this session',
                        upgraded, self._walls_upgraded_session)
            self._emit_state(walls_upgraded = self._walls_upgraded_session)
        return confirmed
    ...
    def _find_match_and_attack(self, method_id, ranked_fill = False):
        ...
        battle_x, _ = self._wait_for_any_image(('surrender.png', 'endbattle.png'), timeout = 30)
        ...
        try:
            ...
            logger.warning('Battle deployment incomplete; continuing battle recovery: %s', exc)
            result = None
...
'''
    ...
    def _wait_for_image(self, template, timeout = 10, error = True, region = None, y_anchor = None, y_slop = 200, threshold = 0.8):
        ...
        if template in ('attack.png', 'farmbattle.png') and last_search is not None:
            self.vision.save_attack_diagnostic(*last_search, threshold=threshold, template_name=template)
        return (None, None)
