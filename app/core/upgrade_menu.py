...
_BUILDER_CHIP_ROI_FRAC = (0.492, 0.028, 0.048, 0.043)  # x, y, w, h as fractions of the frame
...
def _parse_fraction_chip(frame, roi_frac, max_total):
    ...
    for upscale, white in ((6, True), (8, True), (6, False)):
        ...
        if parsed is not None:
            return parsed
    return _parse_fraction_chip_threshold(frame, roi, max_total)


def _parse_fraction_chip_threshold(frame, roi, max_total):
    '''Fallback for outlined HUD glyphs that character-level OCR merges into a slash.'''
    ...
    for threshold in (140, 160):
        mono = cv2.threshold(gray, threshold, 255, cv2.THRESH_BINARY)[1]
        try:
            text = pytesseract.image_to_string(
                mono, config = '--psm 7 -c tessedit_char_whitelist=0123456789/'
            ).strip().replace(' ', '')
        except Exception:
            logger.debug('Builder chip fallback OCR failed', exc_info = True)
            return None
        if '/' in text:
            ...
            digits = (compact[0], compact[1]) if len(compact) == 2 else ('', '')
        if all(value.isdigit() for value in digits):
            free, total = (int(value) for value in digits)
