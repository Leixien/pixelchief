...
from typing import Any, Dict, Optional, Tuple, List, Sequence, Iterator
...
from app.utils.common import ensure_dir, get_resource_path, get_template_path, get_user_app_data_dir
...
try:
    ...
    class VisionService:
        ...
        _last_attack_diagnostic: dict[str, float] = {}

        @staticmethod
        def _deployment_border(
            screen_img: np.ndarray, overlay: np.ndarray | None = None,
        ) -> list[tuple[int, int]]:
            '''Return visible grass points outside a supported red boundary, or none.'''
            if screen_img is None or not screen_img.size:
                return []
            h, w = screen_img.shape[:2]
            scale = min(w / 1280, h / 720)
            hsv = cv2.cvtColor(screen_img, cv2.COLOR_BGR2HSV)
            red = cv2.inRange(hsv, (0, 100, 90), (18, 255, 255))
            red |= cv2.inRange(hsv, (170, 100, 90), (180, 255, 255))
            usable = np.zeros((h, w), dtype=np.uint8)
            usable[int(h * .10):int(h * .70), int(w * .04):int(w * .96)] = 255
            usable[:int(h * .31), :int(w * .19)] = 0
            usable[:int(h * .31), int(w * .80):] = 0
            usable[int(h * .58):, int(w * .80):] = 0
            usable[int(h * .68):, :int(w * .48)] = 0
            red &= usable
            lines = cv2.HoughLinesP(red, 1, np.pi / 180, max(15, int(25 * scale)),
                                    minLineLength=w * .03, maxLineGap=12 * scale)
            if lines is None:
                logger.info('ADB border rejected: no colored line segments')
                return []
            if lines.size == 0:
                logger.info('ADB border rejected: empty line array')
                return []
            if lines.ndim == 0 or lines.shape[-1] != 4 or lines.size % 4:
                logger.warning('Unexpected deployment line array shape: %s', lines.shape)
                return []
            segments = []
            for x1, y1, x2, y2 in lines.reshape(-1, 4):
                dx, dy = int(x2 - x1), int(y2 - y1)
                if dx and .40 <= abs(dy / dx) <= .95:
                    segments.append(((int(x1), int(y1)), (int(x2), int(y2))))
            groups = []
            for endpoints in sorted(segments, key=lambda pair: math.dist(*pair), reverse=True):
                a, b = np.array(endpoints, dtype=float)
                if a[0] > b[0]:
                    a, b = b, a
                direction = (b - a) / np.linalg.norm(b - a)
                for group in groups:
                    origin, axis, low, high = group
                    if float(np.dot(direction, axis)) < .999:
                        continue
                    normal = np.array([-axis[1], axis[0]])
                    if max(abs(np.dot(a - origin, normal)), abs(np.dot(b - origin, normal))) > 5 * scale:
                        continue
                    start, end = sorted((float(np.dot(a - origin, axis)), float(np.dot(b - origin, axis))))
                    if start > high + w * .04 or end < low - w * .04:
                        continue
                    group[2], group[3] = min(low, start), max(high, end)
                    break
                else:
                    groups.append([a, direction, 0., float(np.linalg.norm(b - a))])
            raw_count = len(segments)
            segments = []
            for origin, axis, low, high in groups:
                if high - low >= w * .08:
                    pair = np.rint([origin + axis * low, origin + axis * high]).astype(np.int32)
                    segments.append(pair)
                    if overlay is not None:
                        cv2.line(overlay, tuple(map(int, pair[0])), tuple(map(int, pair[1])), (255, 180, 0), 1)
            #logger.info('ADB border: diagonal segments=%d grouped long segments=%d', raw_count, len(segments))
            if len(segments) < 3:
                #logger.info('ADB border rejected: fewer than three grouped segments')
                return []
            hull = cv2.convexHull(np.array(segments, dtype=np.int32).reshape(-1, 2))
            area = cv2.contourArea(hull)
            if not w * h * .12 <= area <= w * h * .65:
                #logger.info('ADB border rejected: hull area fraction=%.3f', area / (w * h))
                return []
            vertices = hull.reshape(-1, 2)
            if len(vertices) < 3:
                #logger.info('ADB border rejected: degenerate hull')
                return []
            center = vertices.mean(axis=0)
            support = cv2.dilate(red, np.ones((3, 3), dtype=np.uint8))
            grass = cv2.inRange(hsv, (22, 45, 55), (85, 255, 255))
            # ponytail: conservative visible edges only; use terrain segmentation if coverage is insufficient.
            offset, spacing = max(6, int(12 * scale)), max(12, int(28 * scale))
            points = []
            supported_edges = 0
            for edge_index, (a, b) in enumerate(zip(vertices, np.roll(vertices, -1, axis=0))):
                delta = b - a
                length = float(np.linalg.norm(delta))
                if length < w * .10:
                    #logger.info('ADB border edge %d rejected: length=%.1f', edge_index, length)
                    continue
                normal = np.array([-delta[1], delta[0]], dtype=float) / length
                if np.dot(normal, (a + b) / 2 - center) < 0:
                    normal = -normal
                samples = [a + t * delta for t in np.linspace(.05, .95, 40)]
                hits = sum(support[int(p[1]), int(p[0])] != 0 for p in samples)
                if hits < 28:
                    #logger.info('ADB border edge %d rejected: colored support=%d/40', edge_index, hits)
                    continue
                if overlay is not None:
                    cv2.line(overlay, tuple(map(int, a)), tuple(map(int, b)), (0, 255, 255), 2)
                edge_points = []
                for t in np.linspace(.08, .92, max(2, int(length / spacing))):
                    p = a + t * delta + normal * offset
                    x, y = int(round(p[0])), int(round(p[1]))
                    radius = max(2, int(3 * scale))
                    if not radius <= x < w - radius or not radius <= y < h - radius:
                        continue
                    if not usable[y, x] or cv2.pointPolygonTest(hull, (float(x), float(y)), False) >= 0:
                        continue
                    patch = grass[y-radius:y+radius+1, x-radius:x+radius+1]
                    if cv2.countNonZero(patch) < patch.size * .8:
                        continue
                    if np.any(red[y-radius:y+radius+1, x-radius:x+radius+1]):
                        continue
                ...
                if edge_points:
                    ...
                    logger.info('ADB border edge %d rejected: no visible exterior grass points', edge_index)
            #logger.info('ADB border: accepted edges=%d exterior points=%d', supported_edges, len(points))
            if supported_edges < 1 or len(points) < 3:
                logger.info('ADB border rejected: need one supported edge and three exterior points')
            return points if supported_edges >= 1 and len(points) >= 3 else []

        @staticmethod
        def _deployment_camera_stable(reference: np.ndarray, current: np.ndarray) -> bool:
            '''Require distinct stationary terrain patches across multiple rows and columns.'''
            ...
            scale = min(w / 1280, h / 720)
            radius, margin = max(12, round(28 * scale)), max(8, round(24 * scale))
            tolerance = max(1, round(2 * scale))
            before = cv2.cvtColor(reference, cv2.COLOR_BGR2GRAY)
            after = cv2.cvtColor(current, cv2.COLOR_BGR2GRAY)
            stationary = []
            for row, fy in enumerate((.24, .40, .56)):
                for col, fx in enumerate((.28, .50, .72)):
                    x, y = round(w * fx), round(h * fy)
                    extent = radius + margin
                    if x < extent or y < extent or x + extent >= w or y + extent >= h:
                        continue
                    patch = before[y-radius:y+radius, x-radius:x+radius]
                    if float(patch.std()) < 12:
                        continue
                    search = after[y-extent:y+extent, x-extent:x+extent]
                    matches = cv2.matchTemplate(search, patch, cv2.TM_CCOEFF_NORMED)
                    _, score, _, location = cv2.minMaxLoc(matches)
                    if score < .90:
                        continue
                    mx, my = location
                    alternatives = matches.copy()
                    exclusion = tolerance + 2
                    alternatives[max(0, my-exclusion):my+exclusion+1,
                                 max(0, mx-exclusion):mx+exclusion+1] = -1
                    if score - float(alternatives.max()) < .05:
                        continue
                    if max(abs(mx - margin), abs(my - margin)) > tolerance:
                        logger.info('ADB camera: terrain movement detected at patch %d,%d', row, col)
                        return False
                    stationary.append((row, col))
            stable = (len(stationary) >= 4 and len({r for r, _ in stationary}) >= 2
                      and len({c for _, c in stationary}) >= 2)
            #logger.info('ADB camera: stationary patches=%d/9 stable=%s', len(stationary), stable)
            return stable

        @staticmethod
        def _deployment_count(screen_img: np.ndarray, center: tuple[int, int]) -> int | None:
            '''Read the quantity above the selected portrait; missing text is not zero.'''
            if screen_img is None or not screen_img.size:
                return None
            h, w = screen_img.shape[:2]
            x, y = center
            left, right = max(0, x - round(w * .025)), min(w, x + round(w * .031))
            top, bottom = max(0, y - round(h * .080)), min(h, y - round(h * .049))
            ...
            try:
                words = VisionService.find_words_ocr(
                    screen_img, region,
                    white_text=True, cc_filter_blobs=False, roi_upscale=3,
                    tesseract_config='--psm 7 -c tessedit_char_whitelist=xX0123456789',
                )
                raw = ''.join(word.text.strip() for word in sorted(words or [], key=lambda word: word.left))
                match = re.fullmatch(r'[xX]([0-9]{1,3})', raw)
                if match:
                    value = int(match.group(1))
                    if 0 <= value <= 400:
                        return value
                #logger.info('ADB quantity unreadable: region=%s text=%r', region, raw)
                return None
            except Exception:
                logger.warning('Cannot read deployment quantity: region=%s', region, exc_info=True)
                return None

        @staticmethod
        def save_attack_diagnostic(
            screen_img: np.ndarray, region: tuple[int, int, int, int] | None = None,
            threshold: float = 0.8, *, template_name: str = 'attack.png',
        ) -> None:
            '''Save the failed search and scale comparisons; never affect recognition.'''
            if template_name not in ('attack.png', 'farmbattle.png'):
                return
            stem = Path(template_name).stem
            now = time.monotonic()
            last = VisionService._last_attack_diagnostic.get(template_name)
            if last is not None and now - last < 60:
                return
            VisionService._last_attack_diagnostic[template_name] = now
            try:
                cfg = Config()
                ...
                template = VisionService._resize_template_for_screen(original, screen_img)
                sh, sw = screen_img.shape[:2]
                x, y, w, h = region if region is not None else (0, 0, sw, sh)
                if min(x, y) < 0 or min(w, h) <= 0 or x + w > sw or y + h > sh:
                    raise ValueError(f'Invalid search region: {region}')
                search = screen_img[y:y + h, x:x + w]
                th, tw = template.shape[:2]
                matches = []
                nominal = None
                for factor in (0.5, 0.75, 1.0, 1.25, 1.5, 2.0):
                    mw, mh = max(1, round(tw * factor)), max(1, round(th * factor))
                    if mw > w or mh > h:
                        continue
                    scaled = template if factor == 1.0 else cv2.resize(
                        original, (mw, mh), interpolation=(
                            cv2.INTER_AREA if mw < original.shape[1] else cv2.INTER_CUBIC
                        ),
                    )
                    result = cv2.matchTemplate(search, scaled, cv2.TM_CCOEFF_NORMED)
                    _, score, _, location = cv2.minMaxLoc(result)
                    match = {
                        'relative_scale': factor, 'score': float(score),
                        'box_xywh': [x + location[0], y + location[1], mw, mh],
                    }
                    matches.append(match)
                    if factor == 1.0:
                        nominal = match
                if nominal is None:
                    raise ValueError(f'Scaled {template_name} template is larger than the search region.')
                bx, by, bw, bh = nominal['box_xywh']
                candidate = screen_img[by:by + bh, bx:bx + bw]
                directory = get_user_app_data_dir() / 'debug'
                ensure_dir(directory)
                for filename, image in (
                    (f'{stem}_screen.png', screen_img),
                    (f'{stem}_template.png', template),
                    (f'{stem}_candidate.png', candidate),
                ):
                    if not cv2.imwrite(str(directory / filename), image):
                        raise OSError(f'Cannot write {directory / filename}')
                metadata = {
                    'saved_at_unix': time.time(), 'template_path': str(path),
                    'profile': cfg.aspect_key, 'screen_wh': [sw, sh],
                    'reference_wh': [cfg.ref_width, cfg.ref_height],
                    'template_original_wh': [original.shape[1], original.shape[0]],
                    'template_scaled_wh': [tw, th], 'region_xywh': [x, y, w, h],
                    'threshold': threshold, 'nominal_match': nominal,
                    'scale_comparisons': matches,
                    'candidate_image': 'Nominal-scale best match, not a confirmed button.',
                }
                (directory / f'{stem}_match.json').write_text(
                    json.dumps(metadata, indent=2, allow_nan=False), encoding='utf-8',
                )
                best = max(matches, key=lambda match: match['score'])
                logger.warning(
                    '%s diagnostic: profile=%s screen=%sx%s template=%sx%s '
                    'region=%s score=%.4f threshold=%.2f best_scale=%.2f best_score=%.4f files=%s',
                    stem.capitalize(), cfg.aspect_key, sw, sh, tw, th, (x, y, w, h), nominal['score'],
                    threshold, best['relative_scale'], best['score'], directory,
                )
            except Exception:
                logger.warning('Could not save %s diagnostic', stem, exc_info=True)
        ...
        @staticmethod
        def _template_match_maps(
            original: np.ndarray, screen_img: np.ndarray, search_img: np.ndarray,
            threshold: float | None = None, *, scale_template: bool = True,
        ) -> Iterator[tuple[np.ndarray, int, int]]:
            '''Yield nominal matching first, then ADB alternatives when needed.'''
            nominal = (
                VisionService._resize_template_for_screen(original, screen_img)
                if scale_template else original
            )
            nh, nw = nominal.shape[:2]
            ...
            for factor in factors:
                ...
                if (tw, th) in seen or tw > sw or th > sh:
                    continue
                seen.add((tw, th))
                template = nominal if factor == 1.0 else cv2.resize(
                    original, (tw, th), interpolation=(
                        cv2.INTER_AREA if min(tw / original.shape[1], th / original.shape[0]) < 1
                        else cv2.INTER_CUBIC
                    ),
                )
                result = cv2.matchTemplate(search_img, template, cv2.TM_CCOEFF_NORMED)
                nominal_sufficient = (
                    factor == 1.0 and threshold is not None
                    and cv2.minMaxLoc(result)[1] >= threshold
                )
                yield result, tw, th
                if nominal_sufficient:
                    return
        ...
        def _best_template_match(
            ...
            best = (0.0, None, 0, 0)
            for result, tw, th in VisionService._template_match_maps(
                original, screen_img, search_img, threshold, scale_template=scale_template,
            ):
                _, score, _, location = cv2.minMaxLoc(result)
                if best[1] is None or score > best[0]:
                    best = (float(score), location, tw, th)
            return best
...
'''
            ...
            try:
                ...
                max_val, max_loc, t_w, t_h = VisionService._best_template_match(
                    template, screen_img, search_img, threshold, scale_template=scale_template,
                )
                if max_loc is None:
                    return (None, None)
...
'''
            ...
            try:
                ...
                if region:
                    ...
                    (offset_x, offset_y) = (0, 0)
                max_val, max_loc, t_w, t_h = VisionService._best_template_match(
                    template, screen_img, search_img, threshold if threshold > 0 else None,
                )
                if max_loc is None:
                    return (None, None, 0)
                if max_val < threshold:
        ...
        def _template_best_match(screen_img, template_name, region = None):
            ...
            try:
                ...
                if region:
                    ...
                    (offset_x, offset_y) = (0, 0)
                max_val, max_loc, t_w, t_h = VisionService._best_template_match(
                    template, screen_img, search_img,
                )
                if max_loc is None:
                    return (0, None, None)
                center_x = offset_x + max_loc[0] + t_w // 2
...
'''
            ...
            if not matches:
                buttons = VisionService._find_addwall_labels(screen_img, region)
                return max(buttons, key = lambda button: button[0]) if buttons else (None, None)
            ...
            for symbol_x, symbol_y in sorted(passing, key=lambda pt: pt[0], reverse=True):
                button = (
                    max(0, symbol_x - round(frame_w * .017)),
                    min(frame_h - 1, symbol_y + round(frame_h * .018)),
                )
                if not VisionService._wall_button_context_valid(screen_img, button):
                    continue
                logger.info('Add Wall symbol=(%d,%d) button=%s', symbol_x, symbol_y, button)
                return button
            buttons = VisionService._find_addwall_labels(screen_img, region)
            return max(buttons, key = lambda button: button[0]) if buttons else (None, None)

        @staticmethod
        def _find_addwall_labels(screen_img, region):
            '''Find the level-independent Add Wall label when the wall icon changes.'''
            words = VisionService.find_words_ocr(
                screen_img, region = region, preprocess = True, white_text = True,
                brightness_floor = 160, min_confidence = 15,
                tesseract_config = '--psm 11')
            lines = VisionService.cluster_ocr_boxes_by_y(
                words, max(10, round(screen_img.shape[0] * .02)))
            buttons = []
            for line in lines:
                ordered = sorted(line, key = lambda word: word.left)
                for start in range(len(ordered)):
                    for end in range(start + 1, min(start + 3, len(ordered)) + 1):
                        group = ordered[start:end]
                        text = ''.join(
                            c for word in group for c in word.text.lower() if c.isalnum())
                        if difflib.SequenceMatcher(None, text, 'addwall').ratio() < .75:
                            continue
                        ...
                        button = (
                            (left + right) // 2,
                            max(0, (top + bottom) // 2 - round(screen_img.shape[0] * .03)))
                        logger.info('Add Wall label=%r button=%s', text, button)
                        buttons.append(button)
            return buttons

        @staticmethod
        def _wall_button_context_valid(screen_img, center):
            '''Reject colored digits on terrain; wall controls have a large pale button face.'''
            h, w = screen_img.shape[:2]
            x, y = center
            half_w, half_h = max(12, round(w * .04)), max(12, round(h * .063))
            if x < half_w or y < half_h or x + half_w >= w or y + half_h >= h:
                return False
            crop = screen_img[y-half_h:y+half_h, x-half_w:x+half_w]
            hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
            pale = cv2.inRange(hsv, (0, 0, 150), (179, 120, 255))
            return cv2.countNonZero(pale) >= pale.size * .45
...
'''
            ...
            if not matches:
                add_buttons = VisionService._find_addwall_labels(screen_img, region)
                if add_buttons:
                    add_x, add_y = min(add_buttons, key = lambda button: button[0])
                    button = (add_x - round(screen_img.shape[1] * .11), add_y)
                    if VisionService._wall_button_context_valid(screen_img, button):
                        return button
                return (None, None)
            (frame_h, frame_w) = screen_img.shape[:2]
            passing = []
            for left, top, tw, th, _score in matches:
                ...
                if VisionService.yellow_fraction(crop) < yellow_threshold:
                    continue
                passing.append((int(left) + int(tw) // 2, int(top) + int(th) // 2))
            for symbol_x, symbol_y in sorted(passing, key=lambda pt: pt[0]):
                button = (
                    max(0, symbol_x - round(frame_w * .014)),
                    min(frame_h - 1, symbol_y + round(frame_h * .020)),
                )
                if not VisionService._wall_button_context_valid(screen_img, button):
                    continue
                logger.info('Remove Wall symbol=(%d,%d) button=%s', symbol_x, symbol_y, button)
                return button
            add_buttons = VisionService._find_addwall_labels(screen_img, region)
            if add_buttons:
                add_x, add_y = min(add_buttons, key = lambda button: button[0])
...
'''
            ...
            try:
                ...
                if region:
                    ...
                    (offset_x, offset_y) = (0, 0)
                max_val, max_loc, t_w, t_h = VisionService._best_template_match(
                    template, screen_img, search_img, threshold,
                )
                if max_loc is None:
                    return (None, None, None, None, 0)
                score = float(max_val)
        ...
        def _find_template_matches(screen_img, template_name, threshold, region = None, *, max_matches = 2, suppress_pad_frac = 0.5):
            ...
            try:
                ...
                scale_count = 0
                for result, t_w, t_h in VisionService._template_match_maps(
                    template, screen_img, search_img,
                ):
                    scale_count += 1
                    work = result.copy()
                ...
                if scale_count <= 1:
                    return matches
                selected = []
                for match in sorted(matches, key=lambda item: item[4], reverse=True):
                    left, top, tw, th, _ = match
                    duplicate = False
                    for other_left, other_top, ow, oh, _ in selected:
                        overlap_w = max(0, min(left + tw, other_left + ow) - max(left, other_left))
                        overlap_h = max(0, min(top + th, other_top + oh) - max(top, other_top))
                        if overlap_w * overlap_h > 0.5 * min(tw * th, ow * oh):
                            duplicate = True
                            break
                    if not duplicate:
        ...
        def _upgrade_cost_redness_for_template_rect(screen_img, left, top, tw, th, conf, *, center_offset, roi_w, roi_h, frame_w, frame_h):
            ...
            hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
            orange_red = cv2.inRange(hsv, (0, 40, 40), (20, 255, 255))
            red = max(red, float(cv2.countNonZero(orange_red)) / float(cw * ch))
...
'''
        ...
        @staticmethod
        def find_upgrade_resource_buttons(screen_img, region = None):
            '''Find gold/elixir payment buttons when the combined multiupgrade template misses.'''
            if screen_img is None or not screen_img.size:
                return {}
            h, w = screen_img.shape[:2]
            if region is None:
                region = (w // 4, h // 2, w // 2, h // 2)
            found = {}
            for resource, template_name in (('gold', 'bgold.png'), ('elixir', 'belixir.png')):
                matches = VisionService._find_template_matches(
                    screen_img, template_name, .70, region, max_matches=8,
                )
                matches = [m for m in matches if m[0] > w * .45 and m[1] > h * .65]
                if matches:
                    left, top, tw, th, score = max(matches, key=lambda m: m[4])
                    found[resource] = (int(left + tw // 2), int(top + th // 2), float(score))
            return found
