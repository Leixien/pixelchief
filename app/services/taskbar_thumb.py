...
def _create_play_icon(size = 32):
    ...
    draw.polygon([
        (left, top),
        (left, bottom),
        (right, mid_y)], fill = (255, 255, 255))
    return _pil_to_hicon(img)


def _create_stop_icon(size = 32):
...
def _pil_to_hicon(img):
    ...
    try:
        os.close(fd)
        img.save(path, format = 'ICO', sizes = [
            (img.width, img.height)])
        path_w = os.path.abspath(path)
        hicon = user32.LoadImageW(None, path_w, IMAGE_ICON, img.width, img.height, LR_LOADFROMFILE | LR_DEFAULTSIZE)
        if not hicon:
...
def _guid_from_str(s):
    ...
    return GUID(int(s[0:8], 16), int(s[8:12], 16), int(s[12:16], 16), (ctypes.c_ubyte * 8)(*[ int(s[16 + i * 2:18 + i * 2], 16) for i in range(8) ]))
...
ThumbBarAddButtons_t = ctypes.WINFUNCTYPE(ctypes.c_long, ctypes.c_void_p, wintypes.HWND, wintypes.UINT, ctypes.POINTER(THUMBBUTTON))
ThumbBarUpdateButtons_t = ctypes.WINFUNCTYPE(ctypes.c_long, ctypes.c_void_p, wintypes.HWND, wintypes.UINT, ctypes.POINTER(THUMBBUTTON))
...
class TaskbarThumb:
    ...
    def _setup_com(self):
        ...
        hr = ole32.CoCreateInstance(ctypes.byref(clsid), None, CLSCTX_INPROC_SERVER, ctypes.byref(iid), ctypes.byref(taskbar))
        if hr != 0:
            logger.warning(f'''CoCreateInstance failed: 0x{hr}''')
            return False
        ...
        if hr != 0:
            logger.warning(f'''HrInit failed: 0x{hr}''')
            return False
        return True
    ...
    def _add_buttons(self):
        ...
        if hr != 0:
            logger.warning(f'''ThumbBarAddButtons failed: 0x{hr}''')
            return False
        return True
